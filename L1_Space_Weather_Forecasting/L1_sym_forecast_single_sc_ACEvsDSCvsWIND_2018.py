# -*- coding: utf-8 -*-
"""
Created on Wed Jan 31 14:16:54 2024
...
df_params = pd.read_csv('dscovr_L1_2018_1min.csv')
...
#%% Plot Bz as a test over 2018 storm range
...
df_sym = pd.read_csv('omni_ind_params_2018_1min.csv')
...
#initial_sym = -40 # test varying initial value
#current_sym = -40
...
plt.xlabel('Day in 2018', fontsize=15)
...
df_params2 = pd.read_csv('wind_L1_2018_1min.csv')  # Change the filename to the new dataset
...
df_storm19_2 = df_params2[(lower_day < df_params2['Day']) & (df_params2['Day'] < upper_day)]
...
plt.plot(days_storm2, Bz_storm2, label='BZ, GSE, nT - Wind')
plt.xlabel('Day in 2018')
plt.ylabel('BZ, GSE, nT')
...
# Filter the DataFrame for the new dataset
df_sym_storm2 = df_sym2[(lower_day < df_sym2['Day']) & (df_sym2['Day'] < upper_day)]
...
# Read in B field and plasma data for the NEW dataset
df_params3 = pd.read_csv('ace_L1_2018_1min.csv')  # Change the filename to the new dataset
...
# Filter the DataFrame for the new dataset
df_storm19_3 = df_params3[(lower_day < df_params3['Day']) & (df_params3['Day'] < upper_day)]
...
# Filter the DataFrame for the new dataset
df_sym_storm3 = df_sym3[(lower_day < df_sym3['Day']) & (df_sym3['Day'] < upper_day)]
