# -*- coding: utf-8 -*-
"""
Created on Thu Feb  1 17:32:33 2024
...
def getData(i, trange): 
    data = []
    progress_bar = tqdm.tqdm(total=100, desc=f"Downloading Data for {nameys[0][i]}")
    ...
    try:
        if i == 4:
            ...
            prick = {
                'dsc_h0_mag_B1GSE': pyspedas.dscovr.mag(trange, datatype='h0', notplot=True)['dsc_h0_mag_B1GSE'],
        ...
        print(f"Error in downloading data for {nameys[0][i]}: {e}")
...
time = pd.read_csv('wind_T1_1min_unix.csv')['Time'].values

#%%

trange=['2015-06-08/00:00:00','2016-06-27/23:59:00']
dlFormat(trange,"SPEDAS_2018_forecast_params_",time)
"""
